...
foreach ($f in $files) {
    ...
    if ($content -match "Follow Me") {
        Write-Host "Skipping $($f.Name) - already has Follow Me"
        continue
    }
    
    if ($content.Contains($waBlock)) {
        $content = $content.Replace($waBlock, $newBlock)
        Set-Content -Path $f.FullName -Value $content
        Write-Host "Updated $($f.Name)"
    } elseif ($content.Contains($waBlockAlt)) {
        $content = $content.Replace($waBlockAlt, $newBlockAlt)
        Set-Content -Path $f.FullName -Value $content
        Write-Host "Updated $($f.Name) (Alt indent)"
    } else {
        Write-Host "WhatsApp block not found in $($f.Name)"
    }
}
